export const postsArr = [
    ...
    },
    {
        ...
        content: "Today I participated in JavaScriptmas (hosted by scrimba). The challenge was to fix a broken gift list site, where the user can add gifts to a list. The user was capable of adding multiple entries that were similar if they added spaces to the item. This was creating duplicates. I went into the existing code and read through it to understand the basics of the app. It was pretty simple. An app that displayed items added to a list based on weather or not that item existed in an array using a simple input and button form. I made my changes in the checkDuplicate() function. I took the grabbed input value and made all the letters lowercase and turned it in to an array (if I inputed 'Hand Lotion' it would now be ['h', 'a', 'n', 'd', ' ', 'l', 'o', 't', 'i', 'o', 'n']). Then I itterated through that array using a forEach and created a string with only letters, no spaces. I then created a new array comparing that string to each entry with the spaces also removed. If they were the same it would return true, otherwise false. I then had an array of trues and falses. If the array included true I would set the isInShoppingList boolean to true, otherwise it would be false. Then if that boolean was true I would push the item to the array and then render the list. I also checked for empty items here so the user couldn't accidentally add a bunch of spaces as an item. I then tackled the stretch goals. One being handling case sensitivity, which I already covered by setting the item text toLowerCase() before creating the letters array. The next stretch goal was to allow the user to delete or edit items. I started by allowing the user to delete the item. I went to the renderList() function and added a buttonEl iconEl and spanEl. I put the iconEl and spanEl in the buttonEl. The buttonEl has an event listener on it that references the removeItem function I created. The iconEl is just a trashIcon from fontAwesome and the spanEl is used for assistive screen reader technologies. I just googled how to do that, but essensially it is hidden visually but the screen reader can still see it. For the removeItem function I accepted a parameter of the giftItem that the delete button is on. I then found the index of that giftItem using indexOf(). Then I spliced the array using the giftItem index. So shoppingListArr.splice(newItemIndex, 1). Then I called the renderList() function so it would reload without the deleted item. The final stretch goal was to allow the user to edit the list item. I went to the renderList() function and added an edit icon from fontAwesome simlilar to how I added the delete button. This one however calls the editItem() function which takes the gift item name as a parameter. Within this function I call the removeItem() function and then add the gift item text to the enter an item input. I then use focus() to focus the user on the input. It isn't the slickest way possible but it was simple and is intuitive enough I think. That's how I solved todays javaScriptmas.",
